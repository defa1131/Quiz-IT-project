(function () {
    const Test = {
        ...
        complete() {
            ...
            // Id выбранных ответов передать через url в одном параметре через запятую.
            const chosenAnswers = this.userResult.map(result => result.chosenAnswerId).join(',');
            ...
            if (xhr.status === 200 && xhr.responseText) {
                ...
                if (result) {
                    location.href = 'result.html?score=' + result.score + '&total=' + result.total + '&chosen_answers=' + chosenAnswers;
                }
